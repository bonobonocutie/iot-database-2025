...
from tkinter import * # tkinter는 *로 모든 모듈을 부를 수 없음
...
def modData():
    ...
    if std_id == '':
        messagebox.showwarning('UPDATE', '수정할 데이터를 선택요망!')
        return
...
myFont = Font(family='NanumGothic', size=10) # 이후 화면 위젯에 지정할 동일 폰트 생성
...
ent1.config(state='readonly', foreground='blue') # 값을 입력못하게 방지
...
## 6. 트리뷰 항목 더블클릭하면 이벤트발생(getData() 함수 호출)
