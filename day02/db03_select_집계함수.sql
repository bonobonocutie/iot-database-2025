...
GROUP BY custid
HAVING 총수량 > 1; -- COUNT(*)와 별명을 사용할 수 있음.
...
SELECT c.name, b.bookname
  FROM Customer AS c, Orders AS o, Book AS b
 WHERE c.custid = o.custid AND b.bookid = o.bookid -- JOIN을 위한 조건
		AND b.price = 20000;
        
-- 외부조인 : 조건을 만족하지 않는(일치하지 않는) 데이터도 출력이 필요할 때 사용하는 조인
-- 3-27 : 도서를 구매하지 않은 고객을 포함해 고객의 이름과 고객이 주문한 도서판매가격을 구하시오.
-- LEFT OUTER JOIN 또는 RIGHT OUTER JOIN - LEFT, RIGHT는 기준이 되는 테이블 위치
SELECT *
FROM Customer AS c LEFT JOIN  Orders AS o ON c.custid = o.custid;

-- RIGHT JOIN으로 하면?
SELECT *
FROM Customer AS c RIGHT JOIN  Orders AS o ON c.custid = o.custid;
